...
const extraerDisco = (datos) => {
    const tamanhoBaseDeDatos = discos.length;
    const ultimoId = discos[tamanhoBaseDeDatos - 1]?.id ?? 0;
    const nuevoId = ultimoId === 0 ? ultimoId : ultimoId + 1;
    const nombre = datos.filmName;
    const tipoDeRotacion = datos.rotationType;
    const region = datos.region;
    const minutos = datos.lengthMinutes;
    const formato = datos.videoFormat;
    return {
        id: nuevoId,
        filmName: nombre,
...
const gestorDeErrores = (err, req, res, next) => {
